...
# A role-confused subagent (Task tool child) can echo a delegation-handoff
# placeholder verbatim instead of executing the assigned task. This matches the
# observed Korean placeholder phrasing so _finalize_result can log a warning.
# Adjust or extend for other locales as needed.
...
class SdkBridge:
    ...
    def _is_placeholder_flake(content: str) -> bool:
        ...
        Returns True when the final content matches a known placeholder pattern
        where a role-confused subagent echoes delegation-visibility prose
        instead of executing the assigned task.
    ...
    async def _flush_proactive(self, user_id: int, state: _UserStreamState) -> None:
        ...
        if not content:
            return
        # An injected background turn may decide there is nothing worth telling
        # the owner (no-op review). The agent signals that by ending the turn
        # with the bare sentinel; suppress the push entirely.
        ...
        # The agent may also emit a report body and END with the sentinel line
        # ("... details in the log.\nNO_PUSH") -- the instruction prose says
        # "end your output with NO_PUSH", so accept a trailing sentinel too.
        # Intent is silence either way.
    ...
    async def _finalize_result(
        ...
        # role-confusion: it reported a delegation placeholder instead of
        # executing). Annotates the log with tool_use_count so the caller can
        # distinguish the <=1 (original) from the >1 read-only case.
        if req.synthetic_response is None and self._is_placeholder_flake(content):
            logger.warning(
                ...
                "placeholder pattern -- subagent likely echoed a delegation "
    ...
    async def ensure_owner_stream(
        ...
        """Bootstrap the owner's live stream when none exists yet.
    ...
    async def inject_background_turn(self, user_id: int, text: str) -> bool:
        """Inject a background/cron notification as a turn into the user's
        LIVE session, with no pending request attached.
        ...
        - no live stream exists for this user yet (bot just started); the
          caller bootstraps it via ensure_owner_stream and retries, or
