...
# (two-layer watchdog, layer 2). Called from install.sh (auto service mode)
# and update.sh.
#
...
setup_macos() {
  ...
  # Never register a plist still carrying mint placeholders -- the label/paths
  # would be literal __AGENT_NAME__/__PROJECT_ROOT__ junk.
...
setup_linux() {
  ...
  mkdir -p "$unit_dir"
  cat > "$unit_dir/claude-code-telegram-watchdog.service" <<UNIT
[Unit]
Description=claude-code-telegram bridge polling watchdog

[Service]
Type=oneshot
ExecStart=/bin/bash $PROJECT_ROOT/bridge/watchdog.sh --unit claude-code-telegram.service
UNIT
  cat > "$unit_dir/claude-code-telegram-watchdog.timer" <<UNIT
[Unit]
Description=Run the claude-code-telegram bridge polling watchdog every 2 minutes
...
UNIT
  ...
  systemctl --user enable --now claude-code-telegram-watchdog.timer 2>/dev/null \
    || warn "enable claude-code-telegram-watchdog.timer reported an error"
  if systemctl --user is-active --quiet claude-code-telegram-watchdog.timer 2>/dev/null; then
    info "watchdog timer registered: claude-code-telegram-watchdog.timer (every 2 min)"
  else
    warn "could not verify claude-code-telegram-watchdog.timer is active"
    warn "enable manually: systemctl --user enable --now claude-code-telegram-watchdog.timer"
  fi
  return 0
}
