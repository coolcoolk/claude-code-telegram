#!/bin/bash
# bridge/watchdog.sh -- external polling watchdog (two-layer watchdog, layer 2).
#
...
notify() {
  # Best-effort user notification. This OSS repo has no bundled push script;
  # log the message and return. Operators who want Telegram alerts can wire
  # their own notifier here (e.g. a curl call to the Bot API) or supply a
  # push.sh at $PROJECT_ROOT/routines/push.sh -- if found, it is invoked.
  log "notify: $1"
...
# unreachable (the /run/user shadowing landmine, or a dead user manager).
# Distinct from an unknown/unregistered unit.
is_bus_error() {
  case "$1" in
...
# self-heal it), then notify ONCE per incident via the bus-independent path.
# The marker is cleared by clear_busdown() on the next healthy probe.
handle_busdown() {
  local uid; uid="$(id -u)"
...
# Never kick a service that is not actually registered (fresh installs in
# manual mode, renamed labels). Verify the target exists first.
if [ -n "$LABEL" ]; then
